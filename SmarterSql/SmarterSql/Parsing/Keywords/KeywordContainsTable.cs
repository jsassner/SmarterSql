...
namespace Sassner.SmarterSql.Parsing.Keywords {
	public class KeywordContainsTable {
		/// <summary>
		...
		/// </summary>
		/// <returns></returns>
		public static bool ParseContainsTable(Parser parser, StatementSpans currentStartSpan, List<TokenInfo> lstTokens, ref int i, List<SysObject> lstSysObjects, int startStatementIndex, ref int sysObjectId, out SysObject addedSysObject, out TableSource addedTableSource, TokenInfo startToken) {
			...
			if (null != nextToken && nextToken.Kind == TokenKind.KeywordContainsTable) {
